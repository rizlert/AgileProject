...
public class HomePage {
	...
	public void SearchingWithValidData () {
		...
		if (ActualResult.equals(ExpectedResults)) {
			System.out.println("Search Box & Button is Entered valid Data & recieved valid Results");
		}
		else {
	...
	if (ActualResult.equals(ExpectedResult)) {
		System.out.println("Pass: Entered invalid data & No results Founds ");
	}
	else {
	...
	public void DifferentLanguagesLinks () throws InterruptedException {
		...
		for (String link : LanguageUrls ) {
			...
			driver.get(link);
			System.out.println("Languages URL are " + driver.getCurrentUrl());
			Thread.sleep(2000);
			driver.close();
